    ...
    async def async_setup(self) -> bool:
        ...
        try:
            ...
            self._websocket = HisenseWebSocket(
                self.hass, self.api_client, self._handle_ws_message
            )
                ...
                if device and device_key:
                    ...
                    if msg_type == "status_wifistatus":
                        ...
                        if status and isinstance(status, str):
                            try:
                                decoded_bytes = base64.b64decode(status)
                                # Use 'replace' error handler to handle invalid UTF-8 sequences
                                decoded_status = decoded_bytes.decode(
                                ...
                                _LOGGER.debug(
                                    "Failed to decode status (non-critical): %s", e
                                )
